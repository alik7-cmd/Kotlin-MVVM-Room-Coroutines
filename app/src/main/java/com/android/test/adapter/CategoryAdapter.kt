...
class CategoryAdapter(context: Context) :
    BaseRecyclerAdapter<Category, OnRecyclerItemClickListener<Category>, CategoryAdapter.CategoryVH>(
        context
    ) {
    ...
    class CategoryVH(itemView: View) :
        ...
        init {
            ivCategoryDp = itemView.iv_category_dp
            tvCategoryName = itemView.tv_category_name
            cardContent = itemView.card_category_content
        }
